...
Instead, of plotting trends, we can also visualize the Z scores across scales as heatmaps:
...
And likewise, we can also choose to visualize specific cell type. For example, just reference cell type A and its neighbor interactions with cell types A and B:
...
We can filter down the different cell type interactions to those that are significantly colocalized (positive significant Z score for at least one scale), those that are significantly separated (negative significant Z score for at least one scale), or those that shift between being colocalized and separated at different scales (at least one positive and negative significant Z score for different scales).
...
Additionally, it may also be useful to visualize cells that are in a specific region of the dataset. For example, maybe a cell type colocalizes with another cell type at a more macro scale, but only in a particular location of the tissue.
