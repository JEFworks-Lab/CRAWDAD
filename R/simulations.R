#' Simulate background
#'
#' @description Create a uniform background of points, where each point is a given cell type.
#'
#' @param size numeric; number of points to generate
#' @param cts character vector; the different cell types in the background, vector of IDs
#' @param prob numeric vetor; the probability, or proportion of each cell type, summing to 1, a vector of probabilities
#' @param seed numeric; pseudo random seed for the function sample()
#' @param scale numeric; amount to expand the coordinates by
#'
#' @return data.frame; the x, y coordinates of each point, and column type indicating its cell type
#'
#' @export
...
#'
#' @description takes a dataframe of x, y, and cell type (from simulate_background)
#' and selects cells to create new cell type patterns of circles.
#' The circles can actually be an outer ring and an inner core.
#'
#' @param pos dataframe; the background where the patter will be created. It should have columns x, y, and type (typically from simulate_background)
#' @param locs list of 2-d numeric vectors; each vector is the x and y coordinates of a circle center (0-1)
