...
## example: neighbor cell types that are significantly co-localized with Intermediate mesoderm at some scale
results.coloc$`Intermediate mesoderm`

```

```{r}

## example: neighbor cell types that are significantly separated with Intermediate mesoderm at some scale
results.sep$`Intermediate mesoderm`

```

```{r}

## example: neighbor cell types that become significantly co-localized and separated at difference scales with Intermediate mesoderm
results.change$`Intermediate mesoderm`
...
crawdad::plotTrends(results = results["Intermediate mesoderm"])

```

```{r}

crawdad::plotTrends(results = results["Endothelium"])
...
dat <- crawdad::meltResultsList(results, withPerms = FALSE)
d1 <- dat[dat$reference == "Lateral plate mesoderm" & dat$neighbor == "Intermediate mesoderm",]
d2 <- dat[dat$reference == "Endothelium" & dat$neighbor == "Haematoendothelial progenitors",]
## combine the trends into one data.frame, and have the "id" column label the combo, plot both lines on same plot
## turn off the facet wrap so just coloring the two trends, which are labeled using the "id" column
d1$id <- "Lateral plate mesoderm vs Intermediate mesoderm"
d2$id <- "Endothelium vs Haematoendothelial progenitors"
d2 <- dplyr::bind_rows(list(d1, d2))
# resolutions as numeric:
d2 <- d2 %>%
  dplyr::mutate_at(vars(resolution), as.numeric)


plt <- vizTrends(dat = d, facet = FALSE, lines = FALSE) +
  
  ## add in line from the original plot
  ggplot2::geom_path(data = d2,
                     ggplot2::aes(x = resolution, y = Z, color = id), size = 1.5) +
