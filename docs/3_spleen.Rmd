...
c_ct_ngb <- which(df$celltypes == ct_ngb)
c_ct_nbg_notnear <- c_ctn[!c_ctn %in% c_ctn_near]
...
df_freq <- data.frame(celltypes = c(paste(ct_ngb, 'near', ct_ref),
                      ...
                      proportion = c(length(c_ctn_near)/length(c_ct_ngb),
                                    length(c_ctn_notnear)/length(c_ct_ngb)))
