...
p
pdf('rafael_analysis/paper/fsld_CD4folB_podo.pdf')
p 
dev.off()
...
p <- vizTrends(d1, lines = T, withPerms = T, sig.thresh = zsigs)
p
pdf('rafael_analysis/paper/fsld_podofolB_CD4.pdf')
p 
dev.off()
...
plt <- vizTrends(dat = d1, facet = FALSE, id = "neighbor", 
                 title = "CD4 Memory T cells near Fol B cells", 
                 sig.thresh = zsig) +
...
d2 <- dats[grepl(pattern = "CD4 Memory T cells_near_B cells, red pulp", 
                 dats$reference) & dats$neighbor %in% c("Fol B cells", "Podoplanin"),]
plt <- vizTrends(dat = d2, facet = FALSE, id = "neighbor", 
                 title = "CD4 Memory T cells near Neutrophils/Monocytes", 
                 sig.thresh = zsig) +
  ggplot2::scale_x_log10()
# ggplot2::theme(legend.position="none")
plt
...
p <- vizColocDotplot(dat_filtered, reorder = TRUE, zsig.thresh = zsig, zscore.limit = zsig*2) + 
  ...
  theme(legend.position='bottom',
        ...
        legend.box = 'vertical')
p
pdf('rafael_analysis/paper/supp5c_fsld.pdf', height = 7.7, width = 6.05)
p 
dev.off()
...
p
png('rafael_analysis/paper/supp5_fsld_spat_cts.png', height = 1024, width = 1024)
p 
dev.off()
...
p <- vizTrends(dat_filter, lines = T, withPerms = T, sig.thresh = zsigs)
p
pdf('rafael_analysis/paper/replication/fsld_podofolB_CD4.pdf')
p 
dev.off()
...
p <- ggplot() + 
  ...
  theme(legend.position="none")
p
png('rafael_analysis/paper/subsets/podo_fsld_spat_cts.png', height = 1024, width = 1024)
p 
dev.off()
...
saveRDS(binomMat, file="rafael_analysis/paper/supp5_fsld_binomMat500.RDS")
...
saveRDS(subset.list, file="rafael_analysis/paper/supp5_fsld_subset500.RDS")
...
## 8.0865 hours to run
results.subsets
saveRDS(results.subsets, file="rafael_analysis/paper/supp5_fsld_results500.subsets.RDS")
results.subsets <- readRDS("rafael_analysis/paper/supp5_fsld_results500.subsets.RDS")
...
p <- ggplot() + 
  ...
  theme(legend.position="none")
p
png('rafael_analysis/paper/subsets2/fsld_spat_cts_cd4folb2.png', height = 1024, width = 1024)
p 
dev.off()
