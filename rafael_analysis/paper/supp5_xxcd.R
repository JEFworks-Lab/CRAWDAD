...
sort(table(xxcd$celltypes))
set.seed(8) 
cols <- sample(rainbow(length(unique(xxcd$celltypes))))
names(cols) <- unique(xxcd$celltypes)
cols['indistinct'] <- 'grey'

table(xxcd$celltypes)
vi <- xxcd$celltypes %in% c('Podoplanin', 'CD4 Memory T cells', 'Fol B cells')
ggplot(xxcd, aes(x=x, y=y, col=celltypes)) + geom_point(size=0.2, alpha=0.5) + theme_void() +
...
# results <- readRDS("/Users/jeanfan/Downloads/xxcd.pairwise.100-200.folBcombined.results.res100-6000.removeDups.rds")
...
save(results, shuffle.list, file="rafael_analysis/paper/supp5_xxcd.RData")
load("rafael_analysis/paper/supp5_xxcd.RData")
...
# vizAllClusters(cells, xxcd$celltypes)
...
saveRDS(binomMat, file="rafael_analysis/paper/supp5_xxcd_binomMat.RDS")
binomMat <- readRDS("rafael_analysis/paper/supp5_xxcd_binomMat.RDS")
...
saveRDS(subset.list, file="rafael_analysis/paper/supp5_xxcd_subset.RDS")
...
## 8.0865 hours to run
results.subsets
saveRDS(results.subsets, file="rafael_analysis/paper/supp5_xxcd_results.subsets.RDS")
results.subsets <- readRDS("rafael_analysis/paper/supp5_xxcd_results.subsets.RDS")
...
p
pdf('rafael_analysis/paper/xxcd_CD4folB_podo.pdf')
p 
dev.off()
...
p <- vizTrends(d1, lines = T, withPerms = T, sig.thresh = zsigs)
p
pdf('rafael_analysis/paper/xxcd_podofolB_CD4.pdf')
p 
dev.off()
...
annots_temp <- crawdad::selectLabels(df = xxcd,
                                     com = xxcd$celltypes,
...
p
pdf('rafael_analysis/paper/supp5_xxcd_follB_trend.pdf')
p 
dev.off()
...
p <- vizTrends(dat_filter, lines = T, withPerms = T, sig.thresh = zsigs)
p
pdf('rafael_analysis/paper/supp5_xxcd_redpulb_trend.pdf')
p 
dev.off()
...
p <- ggplot() + 
  ...
  theme(legend.position="none")
p
png('rafael_analysis/paper/supp5_xxcd_spat_cts.png', height = 1024, width = 1024)
p 
dev.off()
...
p <- vizTrends(dat_filter, lines = T, withPerms = T, sig.thresh = zsigs)
p
pdf('rafael_analysis/paper/replication/xxcd_podofolB_CD4.pdf')
p 
dev.off()
...
p <- ggplot() + 
  ...
  theme(legend.position="none")
p
png('rafael_analysis/paper/subsets/podo_xxcd_spat_cts.png', height = 1024, width = 1024)
p 
dev.off()
...
saveRDS(binomMat, file="rafael_analysis/paper/supp5_xxcd_binomMat500.RDS")
binomMat <- readRDS("rafael_analysis/paper/supp5_xxcd_binomMat500.RDS")
...
saveRDS(subset.list, file="rafael_analysis/paper/supp5_xxcd_subset500.RDS")
...
p <- ggplot() + 
  ...
  theme(legend.position="none")
p
png('rafael_analysis/paper/subsets500/podo_xxcd_spat_cts_500.png', height = 1024, width = 1024)
p 
dev.off()
