...
sort(table(ngpl$celltypes))
set.seed(8) 
cols <- sample(rainbow(length(unique(ngpl$celltypes))))
names(cols) <- unique(ngpl$celltypes)
cols['indistinct'] <- 'grey'

table(ngpl$celltypes)
vi <- ngpl$celltypes %in% c('Podoplanin', 'CD4 Memory T cells', 'Fol B cells')
ggplot(ngpl, aes(x=x, y=y, col=celltypes)) + geom_point(size=0.2, alpha=0.5) + theme_void() +
...
# results <- readRDS("/Users/jeanfan/Downloads/ngpl.pairwise.100-200.folBcombined.results.res100-6000.removeDups.rds")
...
# vizAllClusters(cells, ngpl$celltypes)
...
saveRDS(binomMat, file="rafael_analysis/paper/supp5_ngpl_binomMat.RDS")
binomMat <- readRDS("rafael_analysis/paper/supp5_ngpl_binomMat.RDS")
...
saveRDS(subset.list, file="rafael_analysis/paper/supp5_ngpl_subset.RDS")
subset.list <- readRDS("rafael_analysis/paper/supp5_ngpl_subset.RDS")
...
## 8.0865 hours to run
results.subsets
saveRDS(results.subsets, file="rafael_analysis/paper/supp5_ngpl_results.subsets.RDS")
results.subsets <- readRDS("rafael_analysis/paper/supp5_ngpl_results.subsets.RDS")
...
annots_temp <- crawdad::selectLabels(df = ngpl,
                                     com = ngpl$celltypes,
...
p
pdf('rafael_analysis/paper/supp5_ngpl_follB_trend.pdf')
p 
dev.off()
...
p <- vizTrends(dat_filter, lines = T, withPerms = T, sig.thresh = zsigs)
p
pdf('rafael_analysis/paper/supp5_ngpl_redpulb_trend.pdf')
p 
dev.off()
...
p <- vizColocDotplot(dat_filtered, reorder = TRUE, zsig.thresh = zsig, zscore.limit = zsig*2) + 
  ...
  theme(legend.position='bottom',
        ...
        legend.box = 'vertical')
p
pdf('rafael_analysis/paper/supp5c_ngpl.pdf', height = 7.7, width = 6.05)
p 
dev.off()
...
p <- vizColocDotplot(dat_filtered, reorder = TRUE, zsig.thresh = zsig, zscore.limit = zsig*2) + 
  ...
  theme(legend.position='bottom',
        ...
        legend.box = 'vertical')
p
pdf('rafael_analysis/paper/supp5c_ngpl_ordered_pkhl.pdf', height = 7.7, width = 6.05)
p 
dev.off()
...
p
png('rafael_analysis/paper/supp5_ngpl_spat_cts.png', height = 1024, width = 1024)
p 
dev.off()
...
p <- vizTrends(dat_filter, lines = T, withPerms = T, sig.thresh = zsigs)
p
pdf('rafael_analysis/paper/replication/ngpl_podofolB_CD4.pdf')
p 
dev.off()
...
p <- vizTrends(dat_filter, lines = T, withPerms = T, sig.thresh = zsigs)
p
pdf('rafael_analysis/paper/replication/ngpl_podoredpulp_CD4.pdf')
p 
dev.off()
...
p <- ggplot() + 
  ...
  theme(legend.position="none")
p
png('rafael_analysis/paper/subsets/podo_ngpl_spat_cts.png', height = 1024, width = 1024)
p 
dev.off()
...
saveRDS(binomMat, file="rafael_analysis/paper/supp5_ngpl_binomMat500.RDS")
...
saveRDS(subset.list, file="rafael_analysis/paper/supp5_ngpl_subset500.RDS")
...
## 8.0865 hours to run
results.subsets
saveRDS(results.subsets, file="rafael_analysis/paper/supp5_ngpl_results500.subsets.RDS")
results.subsets <- readRDS("rafael_analysis/paper/supp5_ngpl_results500.subsets.RDS")
...
p <- ggplot() + 
  ...
  theme(legend.position="none")
p
png('rafael_analysis/paper/subsets500/podo_ngpl_spat_cts_500.png', height = 1024, width = 1024)
p 
dev.off()
...
p <- vizTrends(dat_filter, lines = T, withPerms = T, sig.thresh = zsigs)
p
pdf('rafael_analysis/paper/subsets500/ngpl_podo_near_folB_CD4.pdf')
p 
dev.off()
...
p <- vizTrends(dat_filter, lines = T, withPerms = T, sig.thresh = zsigs)
p
pdf('rafael_analysis/paper/subsets500/ngpl_podo_not_near_folB_CD4.pdf')
p 
dev.off()
