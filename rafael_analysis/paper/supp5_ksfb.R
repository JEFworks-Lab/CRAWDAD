...
p
pdf('rafael_analysis/paper/ksfb_CD4folB_podo.pdf')
p 
dev.off()
...
p <- vizTrends(d1, lines = T, withPerms = T, sig.thresh = zsigs)
p
pdf('rafael_analysis/paper/ksfb_podofolB_CD4.pdf')
p 
dev.off()
...
p <- vizTrends(dat_filter, lines = T, withPerms = T, sig.thresh = zsigs)
p
pdf('rafael_analysis/paper/fig3/ksfb_cd4folb_podo_trend500.pdf')
p 
dev.off()
...
p <- ggplot() + 
  ...
  theme(legend.position="none")
p
png('rafael_analysis/paper/subsets2/ksfb_spat_cts_cd4folb2.png', height = 1024, width = 1024)
p 
dev.off()
